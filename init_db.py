...
def init_db():
    ...
    with db.cursor() as cursor: #DB가 없으면 만들어라.
        sql1= "CREATE TABLE IF NOT EXISTS mydb.`uploadfile` (`filenumber` int(100) NOT NULL AUTO_INCREMENT, `filename` VARCHAR(100) NOT NULL, `filetype` VARCHAR(100) NOT NULL ,`file_show` INT(100),`date` TIMESTAMP DEFAULT NOW(),PRIMARY KEY (`filenumber`))"
...
def add_file(db, filename, filetype):
    with db.cursor() as cursor:
        sql = "insert into mydb.uploadfile values(NULL,%s, %s,0,DEFAULT)"
...
def search_show(db,filename):
    with db.cursor() as cursor:
        sql="select file_show from mydb.uploadfile where filename=%s"
        data=(filename)
        cursor.execute(sql,data)
